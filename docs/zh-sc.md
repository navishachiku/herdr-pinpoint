...
- **搜索整棵树** — `/` 比对完整的 `space / tab / pane` 路径，并把命中的字高亮。
...
| 按键 | 动作 |
| --- | --- |
| 打字 | 比对整条 `space / tab / pane` 路径，命中的字会高亮 |
| `↓` / `↑` | 移到第一条／最后一条；在第一条再按 `↑` 回到输入框 |
| `1`–`9` | 离开输入框后，直接选取该条 |
| `Enter` | 离开输入框并保留结果；已在结果上则发送 |
| `Ctrl-U` | 清空查询 |
| `Esc` | 放弃查询，回到三栏 |

还在打字时不会有任何一条被选取，所以不会有哪一行看起来可以发送。删到空字符串仍留在搜索里，再按一次退格才离开。
