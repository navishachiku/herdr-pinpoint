...
поиск. Этот попап убирает и то, и другое. Выберите pane — его id в Herdr
попадает в промпт, и агент работает ровно с ним.

> Три клавиши, три секунды. Открыть, стрелка, Enter — и нужный pane уже в
> промпте, раньше, чем вы успели бы дописать его описание.
...
- **Набирайте, чтобы фильтровать** — режима поиска нет; начните печатать, и
  колонка сузится.
...
| Клавиша | Действие |
| --- | --- |
| ввод текста | Фильтрует текущую колонку; курсор встаёт на первое совпадение |
| `↑` / `↓` | Перемещает курсор в текущей колонке |
| `→` | Выбрать: активирует элемент под курсором и переходит к его потомкам |
| `1`–`9` | То же, что `→`, для пронумерованного элемента; только пока ничего не набрано |
| `←` | Назад к родительской колонке |
| `PgUp` / `PgDn` | Переключить страницу (9 элементов на странице) |
| `Ctrl-U` | Очистить запрос |
| `Enter` | Ввести элемент под курсором в вызвавший pane и закрыть |
| `Esc` | Очистить запрос; если он уже пуст — закрыть |

Каждая колонка хранит свой запрос. Номерные метки появляются в колонке,
которая их принимает, — той, что справа от последнего активированного
элемента, — и исчезают, пока набирается запрос.
