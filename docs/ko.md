...
모호한 대상은 비용이 두 번 듭니다. 당신이 설명하는 토큰과 agent가 찾으러 가는 토큰입니다. 이 팝업은 둘 다 없앱니다.

> 대화 상대를 지목하는 데 3초면 충분합니다. 설명을 타이핑하는 것보다 빠릅니다.
...
- **트리 전체 검색** — `/`로 `space / tab / pane` 경로 전체와 대조하고 일치 부분을 강조합니다.
...
| --- | --- |
| `↑` / `↓` | 현재 열에서 커서 이동 |
| `→` 또는 `1`–`9` | 해당 항목을 선택하고 하위로 이동 |
| `←` | 상위 열로 돌아가기 |
| `PgUp` / `PgDn` | 페이지 전환(한 페이지 9개) |
| `/` | 검색 |
| `Enter` | 커서에 있는 항목을 호출한 pane에 입력하고 닫기 |
| `Esc` | 닫기 |

검색하면 세 열이 하나의 결과 목록으로 바뀝니다:

| 키 | 동작 |
| --- | --- |
| 입력 | `space / tab / pane` 경로 전체와 대조하고, 일치 부분을 강조 |
| `↓` / `↑` | 첫 번째／마지막 결과로 이동. 첫 번째에서 `↑`를 다시 누르면 입력창으로 |
| `1`–`9` | 입력창을 벗어난 뒤 해당 결과를 선택 |
| `Enter` | 결과를 유지한 채 입력창을 벗어남. 결과 위라면 전송 |
| `Ctrl-U` | 질의 비우기 |
| `Esc` | 질의를 버리고 세 열로 복귀 |

입력하는 동안에는 아무것도 선택되지 않으므로 전송할 수 있어 보이는 행이 없습니다. 지워서 비워도 검색에 남아 있고, Backspace를 한 번 더 누르면 빠져나옵니다.
